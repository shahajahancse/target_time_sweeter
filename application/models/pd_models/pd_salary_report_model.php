...
class pd_salary_report_model extends CI_Model{
	...
	function grid_monthly_salary_sheet_bangla($sal_year_month, $grid_status, $grid_emp_id)
	{
		//print_r($grid_emp_id);
		$i = 1;
		$year  = substr($sal_year_month,0,4);
		...
		$this->db->select('pr_emp_per_info.emp_full_name,pr_emp_per_info.bangla_nam, pr_designation.*, pr_section.sec_name, pr_emp_com_info.emp_join_date,pr_grade.gr_name,pd_pay_scale_sheet.*');
		...
		$this->db->where('pr_emp_per_info.emp_id = pd_pay_scale_sheet.emp_id');
		$this->db->where("pd_pay_scale_sheet.salary_month = '$sal_year_month'");
		//$this->db->where('pr_designation.desig_id = pd_pay_scale_sheet.desig_id');
		$this->db->where('pr_emp_com_info.emp_desi_id = pr_designation.desig_id');
		$this->db->where('pr_emp_com_info.emp_dept_id = pr_dept.dept_id');
		$this->db->where('pr_emp_com_info.emp_sec_id = pr_section.sec_id');
		$this->db->where('pr_emp_com_info.emp_line_id = pr_line_num.line_id');
	...
	function grid_monthly_salary_sheet_com($sal_year_month, $grid_status, $grid_emp_id)
	{
		...
		$this->db->select('pr_emp_per_info.emp_full_name,pr_designation.desig_name, pr_section.sec_name, pr_emp_com_info.emp_join_date,pr_grade.gr_name,pd_pay_scale_sheet_com.*');
		...
		$this->db->where_in('pr_emp_com_info.emp_id', $grid_emp_id);
		$this->db->where('pr_emp_per_info.emp_id = pr_emp_com_info.emp_id');
		$this->db->where('pr_emp_per_info.emp_id = pd_pay_scale_sheet_com.emp_id');
