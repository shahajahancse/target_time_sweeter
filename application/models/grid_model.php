...
class Grid_model extends CI_Model{
	...
	function grid_pay_slip($year_month, $grid_emp_id)
	{
		...
		$this->db->select('pr_emp_com_info.emp_id,pr_emp_com_info.gross_sal,pr_emp_per_info.bangla_nam,pr_emp_per_info.emp_fname,pr_emp_per_info.emp_mname, pr_designation.desig_bangla, pr_emp_com_info.emp_join_date, pr_dept.dept_name, pr_section.sec_bangla, pr_line_num.line_name ,pr_grade.gr_name, pr_id_proxi.proxi_id, pr_emp_add.emp_pre_add, pr_emp_add.emp_par_add, pr_emp_position.posi_name, pd_pay_scale_sheet.* ');
			$this->db->from('pr_emp_per_info');
			$this->db->from('pr_emp_com_info');
			$this->db->from('pr_designation');
			$this->db->from('pr_dept');
			...
			$this->db->or_where_in("pr_emp_com_info.emp_id", $grid_emp_id);
			$this->db->like('pd_pay_scale_sheet.salary_month', $year_month);
			$this->db->where('pr_emp_per_info.emp_id = pr_emp_com_info.emp_id');
			$this->db->where('pr_emp_per_info.emp_id = pr_id_proxi.emp_id');
			$this->db->where('pr_emp_per_info.emp_id = pr_emp_add.emp_id');
			$this->db->where('pr_emp_com_info.emp_desi_id = pr_designation.desig_id');
		...
		else
		{
			foreach ($query->result() as $row) {
				...
				$data['bangla_nam'][] = $row->bangla_nam;
				$data['sec_bangla'][] = $row->sec_bangla;
				$data['desig_bangla'][] = $row->desig_bangla;
				$data['gr_name'][] = $row->gr_name;
				$data['gross_sal'][] = $row->gross_sal;
				$data['basic_sal'][] = $row->basic_sal;
				$data['absent_days'][] = $row->absent_days;
				$data['att_days'][] = $row->att_days;
				$data['house_r'][] = $row->house_r;
				$data['abs_deduction'][] = $row->abs_deduction;
				$data['att_bonus'][] = $row->att_bonus;
				$data['pd_amount'][] = $row->pd_amount;
				$data['pd_bonus_amount'][] = $row->pd_bonus_amount;
				$data['none_work_amount'][] = $row->none_work_amount;
				$data['none_work_allowance'][] = $row->none_work_allowance;
				$data['night_allowance'][] = $row->night_allowance;
				$data['holiday_allowance'][] = $row->holiday_allowance;
				$data['before_after_absent_amount'][] = $row->before_after_absent_amount;
				$data['adv_deduct'][] = $row->adv_deduct;
				$data['deduct_amount'][] = $row->deduct_amount;
				$data['salary_month'][] = $row->salary_month;
	...
	function grid_monthly_salary_sheet_com($sal_year_month, $grid_status, $grid_emp_id)
	{
		...
		$this->db->select('pr_emp_per_info.emp_full_name,pr_designation.desig_name, pr_section.sec_name, pr_emp_com_info.emp_join_date,pr_grade.gr_name,pr_pay_scale_sheet_com.*,pr_emp_com_info.emp_join_date');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_grade');
	...
	function salary_summary($salary_month,$emp_stat)
	{
		...
		$this->db->from('pr_pay_scale_sheet');
		$this->db->where("pr_pay_scale_sheet.salary_month = '$sal_year_month'");
		$this->db->group_by("pr_pay_scale_sheet.sec_id");
		$query = $this->db->get();
		...
		foreach($query->result() as $rows)
		{
			...
			$sec_id = $rows->sec_id;
			$all_data["sec_name"][] = $this->get_section_name($sec_id);
			// For Cash Man Power
			$salary_draw_cash = 1;
			...
			$total_cash_deduction =  $abs_deduction_cash;
			...
			//For Cash HD_deduct
			$column_name = "hd_ded_amount" ;
			$hd_ded_cash = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			$total_cash_deduction = $total_cash_deduction + $hd_ded_cash;
			$all_data["hd_ded_cash"][] = $hd_ded_cash;
			
			//For Bank abs_deduction
			$hd_ded_bank = $this->get_sum_column($column_name,$emp_id_bank,$salary_month);
			$total_bank_deduction = $total_bank_deduction + $hd_ded_bank;
			$all_data["hd_ded_bank"][] = $hd_ded_bank;
			
			
			//For Cash adv_deduct
			$column_name = "adv_deduct" ;
			$adv_deduct_cash = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			$total_cash_deduction = $total_cash_deduction + $adv_deduct_cash;
			$all_data["adv_deduct_cash"][] = $adv_deduct_cash;
			
			//For Bank adv_deduct
			$adv_deduct_bank = $this->get_sum_column($column_name,$emp_id_bank,$salary_month);
			$total_bank_deduction = $total_bank_deduction + $adv_deduct_bank;
			$all_data["adv_deduct_bank"][] = $adv_deduct_bank;
			
			//For Cash Stamp
			$column_name = "stamp" ;
			$stamp_deduct_cash = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			$total_cash_deduction = $total_cash_deduction + $stamp_deduct_cash;
			$all_data["stamp_deduct_cash"][] = $stamp_deduct_cash;
			...
			$cash_total_allowance = $holiday_allowance_cash;
			...
			$bank_total_allowance = $holiday_allowance_bank;
			$all_data["holiday_allowance_bank"][] = $holiday_allowance_bank;	
			
			
			//For Cash Half	holiday_allowance
			$column_name = "half_holiday_allowance" ;
			$half_holiday_allowance_cash = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			$cash_total_allowance = $half_holiday_allowance_cash;
			$all_data["half_holiday_allowance_cash"][] = $half_holiday_allowance_cash;	
			
			//For Bank holiday_allowance_bank
			$half_holiday_allowance_bank = $this->get_sum_column($column_name,$emp_id_bank,$salary_month);
			$bank_total_allowance = $half_holiday_allowance_bank;
			$all_data["half_holiday_allowance_bank"][] = $half_holiday_allowance_bank;
			...
			$cash_total_allowance = $cash_total_allowance + $night_allowance_cash;
			...
			$bank_total_allowance = $bank_total_allowance + $night_allowance_bank;
			$all_data["night_allowance_bank"][] = $night_allowance_bank;
			
			
			//For Cash 	Ariar
			$column_name = "ariar" ;
			$ariar_cash = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			//$cash_total_allowance = $cash_total_allowance + $night_allowance_cash;
			$all_data["ariar"][] = $ariar_cash;
			...
			//$cash_total = $cash_total + $ot_amount_cash;
			$all_data["cash_net_pay"][] = $cash_net_pay;
		}
		return $all_data;
	}
	...
	//========================Start Salary Summary=================
	function salary_summary_com($salary_month,$emp_stat)
	{
		$all_data = array();
		$year_month = date('Y-m', strtotime($salary_month));
		$sal_year_month = "$year_month-01";
		...
		$this->db->from('pr_pay_scale_sheet_com');
		$this->db->where("pr_pay_scale_sheet_com.salary_month = '$sal_year_month'");
		$this->db->group_by("pr_pay_scale_sheet_com.sec_id");
		$query = $this->db->get();
		...
		foreach($query->result() as $rows)
		{
			...
			// For Cash Man Power
			$salary_draw_cash = 1;
			$emp_cash = $this->count_empid_for_salary_com($sec_id,$emp_stat,$salary_month,$salary_draw_cash,"count");
			$all_data["emp_cash"][] = $emp_cash;
			
			// For Bank Man Power
			$salary_draw_bank = 2;
			$emp_bank = $this->count_empid_for_salary_com($sec_id,$emp_stat,$salary_month,$salary_draw_bank,"count");
			$all_data["emp_bank"][] = $emp_bank;
			
			$all_data["emp_cash_bank"][] =$emp_cash + $emp_bank;
				
			// For Cash Emp ID
			$cash_emp_id = $this->count_empid_for_salary_com($sec_id,$emp_stat,$salary_month,$salary_draw_cash,"emp_id");
			foreach($cash_emp_id as $rows)
			{
				 $data[] = $rows->emp_id;
			}
			...
			// For Bank Emp ID
			$bank_emp_id = $this->count_empid_for_salary_com($sec_id,$emp_stat,$salary_month,$salary_draw_bank,"emp_id");
			foreach($bank_emp_id as $rows)
			{
				$data1[] = $rows->emp_id;
			}
			...
			//For Cash gross_sal
			$column_name = "gross_sal" ;
			$gross_sal_cash = $this->get_sum_column_com($column_name,$emp_id_cash,$salary_month);
			$cash_total = $gross_sal_cash;
			$all_data["gross_sal_cash"][] = $gross_sal_cash;
			...
			//For Cash att_bonus
			$column_name = "att_bonus" ;
			$att_bonus_cash = $this->get_sum_column_com($column_name,$emp_id_cash,$salary_month);
			$cash_total = $cash_total + $att_bonus_cash;
			$all_data["cash_att_bonus"][] = $att_bonus_cash;	
			...
			//For Cash abs_deduction
			$column_name = "abs_deduction" ;
			$abs_deduction_cash = $this->get_sum_column_com($column_name,$emp_id_cash,$salary_month);
			$total_cash_deduction =  $abs_deduction_cash;
			$all_data["abs_deduction_cash"][] = $abs_deduction_cash;
			...
			//For Cash adv_deduct
			$column_name = "adv_deduct" ;
			$adv_deduct_cash = $this->get_sum_column_com($column_name,$emp_id_cash,$salary_month);
			$total_cash_deduction = $total_cash_deduction + $adv_deduct_cash;
			$all_data["adv_deduct_cash"][] = $adv_deduct_cash;
			...
			//For Cash Stamp
			$column_name = "stamp" ;
			$stamp_deduct_cash = $this->get_sum_column_com($column_name,$emp_id_cash,$salary_month);
			$total_cash_deduction = $total_cash_deduction + $stamp_deduct_cash;
			$all_data["stamp_deduct_cash"][] = $stamp_deduct_cash;
			...
			//For Cash ot_amount
			$column_name = "ot_amount" ;
			$ot_amount_cash = $this->get_sum_column_com($column_name,$emp_id_cash,$salary_month);
			//$cash_total = $cash_total + $ot_amount_cash;
			$all_data["cash_ot_amount"][] = $ot_amount_cash;	
			...
			//For Cash net_pay
			$column_name = "net_pay" ;
			$cash_net_pay = $this->get_sum_column_com($column_name,$emp_id_cash,$salary_month);
			//$cash_total = $cash_total + $ot_amount_cash;
			$all_data["cash_net_pay"][] = $cash_net_pay;
		}
		return $all_data;
	...
	function grid_factory_summary($salary_month,$emp_stat,$prev_salary_month)
	{
		$data["fixed"]= $this->factory_salary_summary_fixed($salary_month,$prev_salary_month,$emp_stat);
		
		$data["fixed_prev"]= $this->factory_salary_summary_fixed($prev_salary_month,$prev_salary_month,$emp_stat);

		$data["production"]= $this->pd_report_model->factory_production_monthly_salary_summary($salary_month,$prev_salary_month,$emp_stat);

		$data["production_prev"]= $this->pd_report_model->factory_production_monthly_salary_summary($prev_salary_month,$prev_salary_month,$emp_stat);
		return $data;
	}
	
	function factory_salary_summary_fixed($salary_month,$prev_salary_month,$emp_stat)
	{
		$all_data = array();
		$year_month = date('Y-m', strtotime($salary_month));
		$sal_year_month = "$year_month-01";
		...
		$this->db->from('pr_pay_scale_sheet');
		$this->db->where("pr_pay_scale_sheet.salary_month = '$sal_year_month'");
		$this->db->or_where("pr_pay_scale_sheet.salary_month = '$prev_salary_month'");
		$this->db->group_by("pr_pay_scale_sheet.sec_id");
		$query = $this->db->get();
	...
	//========================End Salary Summary=================
	 function count_empid_for_salary($sec_id,$emp_stat,$salary_month,$salary_draw,$check)
	 {
		...
		if($emp_stat !="ALL" )
		{
			$this->db->where("pr_pay_scale_sheet.status_id", $emp_stat);
		}
		$query = $this->db->get();
		if($check == "count")
		{
	...
	function get_sum_column($column_name,$emp_id,$salary_month)
	{
	 ...
	 function count_empid_for_salary_com($sec_id,$emp_stat,$salary_month,$salary_draw,$check)
	 {
		

		$salary_month = substr($salary_month,0,7);
		$sal_year_month = "$salary_month-01";
		$this->db->select('pr_pay_scale_sheet_com.*');
		$this->db->from('pr_pay_scale_sheet_com');
		$this->db->from('pr_emp_com_info');
		$this->db->where('pr_emp_com_info.emp_id = pr_pay_scale_sheet_com.emp_id');
		$this->db->where("pr_pay_scale_sheet_com.salary_month = '$sal_year_month'");
		$this->db->where("pr_pay_scale_sheet_com.sec_id", $sec_id);
		$this->db->where("pr_emp_com_info.salary_draw",$salary_draw);
		if($emp_stat !="ALL" )
		{
			$this->db->where("pr_pay_scale_sheet_com.status_id", $emp_stat);
		}
		$query = $this->db->get();
		if($check == "count")
		{
	...
	function get_sum_column_com($column_name,$emp_id,$salary_month)
	{
		
		$this->db->select_sum($column_name);
		$this->db->from("pr_pay_scale_sheet_com");
		$this->db->where_in('pr_pay_scale_sheet_com.emp_id', $emp_id);
		$this->db->like("salary_month", $salary_month);
		$query = $this->db->get();
		//echo $this->db->last_query();
		$row = $query->row();
