...
class Grid_model extends CI_Model{
	...
	function continuous_incre_report($grid_firstdate,$grid_seconddate,$grid_emp_id)
	{
		...
		foreach($grid_emp_id as $emp_id)
		{
			$this->db->select('prev_emp_id,new_emp_id,prev_dept,new_dept,prev_section,new_section,prev_line, new_line, prev_desig,new_desig,prev_salary,new_salary,effective_month,prev_grade,new_grade');
				...
				if($query->num_rows() != 0)
			{
				foreach ($query->result() as $rows)	
				{
					...
					$prev_grade_name = $this->get_grade_name($rows->prev_grade);
					...
					$data["prev_grade"][] 				= $prev_grade_name;
					...
					$new_grade_name = $this->get_grade_name($rows->new_grade);
	...
	function grid_pay_slip($year_month, $grid_emp_id)
	{
		...
		$this->db->select('pr_emp_com_info.emp_id,pr_emp_com_info.gross_sal,pr_emp_per_info.emp_full_name,pr_emp_per_info.emp_fname,pr_emp_per_info.emp_mname, pr_designation.desig_name, pr_emp_com_info.emp_join_date, pr_dept.dept_name, pr_section.sec_name, pr_line_num.line_name ,pr_emp_com_info.emp_sal_gra_id, pr_id_proxi.proxi_id, pr_emp_add.emp_pre_add, pr_emp_add.emp_par_add, pr_emp_position.posi_name,pr_grade.gr_name, pr_pay_scale_sheet.* ');
			$this->db->from('pr_emp_per_info');
			$this->db->from('pr_emp_com_info');
			$this->db->from('pr_designation');
			$this->db->from('pr_dept');
	...
	{
		$this->db->select('pr_emp_com_info.emp_id,pr_emp_per_info.emp_full_name,pr_emp_per_info.img_source, pr_designation.desig_name, pr_emp_com_info.emp_join_date, pr_dept.dept_name, pr_section.sec_name, pr_line_num.line_name');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_designation');
		$this->db->from('pr_dept');
	...
	function grid_monthly_att_register($year_month, $grid_emp_id,$grid_status)
	{
		...
		$this->db->select('pr_emp_per_info.emp_full_name,pr_designation.desig_name,pr_attn_monthly.*,pr_emp_com_info.emp_join_date');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_attn_monthly');
			$this->db->from('pr_dept');
		...
		if($grid_status == 4)
		{
			...
			$this->db->where("trim(substr(pr_emp_resign_history.resign_date,1,7)) LIKE '$year_month'");
		}
		else if($grid_status == 3)
		{
			$this->db->where('pr_emp_com_info.emp_id = pr_emp_left_history.emp_id');
			$this->db->where("trim(substr(pr_emp_left_history.left_date,1,7)) LIKE '$year_month'");
		}
		$this->db->order_by("pr_emp_com_info.emp_id");
	...
	function save_holiday($grid_firstdate,$holiday_description,$grid_emp_id)
	{$query = $this->all_reguler_emp($grid_emp_id);
		...
		foreach($query->result() as $row)
		{
			$holiday_empid = $row->emp_id;
			 ...
			 $year_month = $grid_firstdate ;
			 $this->db->select("*");
			 $this->db->where('holyday_date',$year_month);
	...
	function grid_monthly_salary_sheet($sal_year_month, $grid_status, $grid_emp_id)
	{
		...
		$this->db->select('pr_emp_per_info.emp_full_name,pr_designation.desig_name, pr_section.sec_name, pr_line_num.line_name,pr_emp_com_info.emp_join_date,pr_grade.gr_name,pr_pay_scale_sheet.*');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_grade');
		$this->db->from('pr_pay_scale_sheet');
	...
	function grid_monthly_salary_sheet_com($sal_year_month, $grid_status, $grid_emp_id)
	{
		...
		$this->db->select('pr_emp_per_info.emp_full_name,pr_designation.desig_name, pr_section.sec_name, pr_line_num.line_name,pr_emp_com_info.emp_join_date,pr_grade.gr_name,pr_pay_scale_sheet_com.*');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_grade');
	...
	//===================================Bank Note Summary ======================================
	function grid_bank_note_req_summary($salary_month,$emp_stat)
	{	
		$all_data = array();
		...
		$this->db->select("sec_id,sec_name");
		//$this->db->where('sec_id', "8");
		$this->db->order_by("sec_name");
		$query = $this->db->get("pr_section");

		foreach($query->result() as $row_main)
		{
			//echo "<tr>";
			//$emp_stat = array('2','3','4','6');
			$data = array();
			...
			//echo $rows->sec_name;
			//echo "</td>";
			$all_data["dept"][] = $row_main->sec_name;
			$sec_id = $row_main->sec_id;
			
			// For Cash Man Power
			$salary_draw_cash = 1;
			$emp_cash = $this->count_empid_for_salary($sec_id,$emp_stat,$salary_month,$salary_draw_cash,"count");
			$all_data["emp_cash"][] = $emp_cash;
			
			// For Bank Man Power
			$salary_draw_bank = 2;
			$emp_bank = $this->count_empid_for_salary($sec_id,$emp_stat,$salary_month,$salary_draw_bank,"count");
			$all_data["emp_bank"][] = $emp_bank;
			
			$all_data["emp_cash_bank"][] =$emp_cash + $emp_bank;
			
			// For Cash Emp ID
			$cash_emp_id = $this->count_empid_for_salary($sec_id,$emp_stat,$salary_month,$salary_draw_cash,"emp_id");
			foreach($cash_emp_id as $rows)
			{
				$data[] = $rows->emp_id;
			}
			...
			// For Bank Emp ID
			$bank_emp_id = $this->count_empid_for_salary($sec_id,$emp_stat,$salary_month,$salary_draw_bank,"emp_id");
			foreach($bank_emp_id as $rows)
			{
				$data1[] = $rows->emp_id;
			}
			...
			$section_bank_note = $this->get_section_bank_note($sec_id,$salary_month,$emp_stat);
			
			$all_data[1000][] = $section_bank_note[1000];
			$all_data[500][] = $section_bank_note[500];
			$all_data[100][] = $section_bank_note[100];
			$all_data[50][] = $section_bank_note[50];
			$all_data[20][] = $section_bank_note[20];
			$all_data[10][] = $section_bank_note[10];
			$all_data[5][] = $section_bank_note[5];
			$all_data[2][] = $section_bank_note[2];
			$all_data[1][] = $section_bank_note[1];
			
			//print_r($section_bank_note[1000] );
			...
			//For Cash Net Pay
			$column_name = "net_pay" ;
			$cash_net_pay = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			//$cash_total = $eot_hour;
			
			$all_data["cash_net_pay"][] = $cash_net_pay + $cash_eot_amount ;
			$cash_net_pay_eot = $cash_net_pay + $cash_eot_amount ;
			//For Bank Net Pay
			$bank_net_pay = $this->get_sum_column($column_name,$emp_id_bank,$salary_month);
			//$bank_total = $eot_hour;
			$all_data["bank_net_pay"][] = $bank_net_pay + $bank_eot_amount;
			$bank_net_pay_eot = $bank_net_pay + $bank_eot_amount ;
			
			$all_data["net_pay_cash_bank"][] = $cash_net_pay_eot + $bank_net_pay_eot;
			
			//echo "$row_main->sec_name===$cash_net_pay***";
			
		}
		//print_r($section_bank_note_design);
		//print_r($section_bank_note );
		return $all_data;
		
	}
	
	function get_section_bank_note($sec_id,$salary_month,$emp_stat)
	{
		...
		$this->db->select('pr_pay_scale_sheet.*,pr_pay_scale_sheet.emp_id');
		
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_pay_scale_sheet');
		
		$this->db->where('pr_pay_scale_sheet.section_id', $sec_id);
		$this->db->like("pr_pay_scale_sheet.salary_month", $salary_month);
		if($emp_stat != "ALL")
		{
			$this->db->where("pr_emp_com_info.emp_cat_id", $emp_stat);
		}
		...
		foreach($query->result() as $rows)
		{
			$net_pay = $rows->net_pay;
			$emp_id = $rows->emp_id;
			$eot_amount 	 =$rows->eot_amount;
			//$ot_rate = $rows->ot_rate; 
			
			//$eot_amount = round($eot_hour * $ot_rate);
			$net_pay_eot = $eot_amount + $net_pay;
			
			$bank_note_data = $this->common_model->bank_note_requisition($net_pay_eot, $bank_notes);
			//print_r($bank_note_data);
			$one_thou 	+=	$bank_note_data[1000];
			$five_hun 	+=	$bank_note_data[500];
			$one_hun 	+=	$bank_note_data[100];
			$fifty 		+=	$bank_note_data[50];
			$twenty		+=	$bank_note_data[20];
			$ten 		+=	$bank_note_data[10];
			$five 		+=	$bank_note_data[5];
			$two 		+=	$bank_note_data[2];
			$one 		+=	$bank_note_data[1];
		...
		$data[2]	=$two;
		$data[1] 	=$one;
		//echo "------------".$one_thou."==".$five_hun."==".$one_hun."==".$fifty."==".$twenty	."==".$ten ."==".$five ."==".$two ."==". $one."<br>"  ;
		// print_r($data);<>
		//print_r($total_notes[$note]);
		return $data;
	}
	...
	function salary_summary($salary_month,$emp_stat)
	{
		...
		foreach($query->result() as $rows)
		{
			...
			$total_bank_deduction = $abs_deduction_bank;// + $abs_deduction_bank;
			...
			$column_name = "stamp" ;
			$total_cash_stam_deduct = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			//echo "<td>";			
			...
			$total_bank_stam_deduct = $this->get_sum_column($column_name,$emp_id_bank,$salary_month);
			//echo "<td>";			
			...
			$column_name = "tax" ;
			$tax_cash = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			...
			$column_name = "tax" ;
			$tax_bank = $this->get_sum_column($column_name,$emp_id_bank,$salary_month);
			...
			//=================Total Cash Others Deduct calculation===============>>
	...
	//========================End Salary Summary=================
	 function count_empid_for_salary($dept_id,$status,$salary_month,$salary_draw,$check)
	 {
	...
	function grid_probaation_pass_list($year_month)
	{
		$probation_period = $this->common_model->get_setup_attributes(8);
		//$year_month = date("Y-m",mktime(0,0,0,$month,1,$year));
		$new_month= $this->common_model->get_prev_month($probation_period,$year_month);
		$emp_sts = 2;
		$this->db->select('pr_emp_com_info.emp_id, pr_emp_per_info.emp_full_name, pr_dept.dept_name, pr_section.sec_name, pr_line_num.line_name, pr_designation.desig_name,  pr_emp_com_info.emp_join_date,pr_grade.gr_name, pr_emp_com_info.gross_sal');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_grade');
			...
			$this->db->from('pr_line_num');
			$this->db->from('pr_designation');
			$this->db->where("trim(substr(pr_emp_com_info.emp_join_date,1,7))='$new_month'");
			$this->db->where("pr_emp_com_info.emp_cat_id",$emp_sts);
			$this->db->where('pr_emp_com_info.emp_desi_id = pr_designation.desig_id');
			$this->db->where('pr_emp_com_info.emp_dept_id = pr_dept.dept_id');
			$this->db->where('pr_emp_com_info.emp_sec_id = pr_section.sec_id');
			$this->db->where('pr_emp_com_info.emp_line_id = pr_line_num.line_id');
	...
	function grid_age_estimation($grid_emp_id)
	{
		...
		$this->db->select('pr_emp_com_info.emp_id, pr_emp_per_info.emp_full_name, pr_dept.dept_name, pr_section.sec_name, pr_line_num.line_name, pr_designation.desig_name,  pr_emp_com_info.emp_join_date,pr_grade.gr_name, pr_emp_com_info.gross_sal, pr_emp_per_info.spouse_name, pr_emp_per_info.no_child,pr_emp_per_info.emp_fname, pr_emp_per_info.emp_mname,pr_emp_per_info.emp_dob,pr_emp_per_info.identificatiion_marks,pr_emp_per_info.national_brn_id,pr_emp_per_info.img_source,pr_emp_add.emp_pre_add,pr_emp_add.emp_par_add');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_grade');
			$this->db->from('pr_dept');
	...
	function grid_emp_detail_info($grid_emp_id)
	{
		$this->db->select('pr_emp_com_info.emp_id, pr_emp_per_info.*, pr_dept.dept_name, pr_section.sec_name, pr_line_num.line_name, pr_designation.desig_name,  pr_emp_com_info.emp_join_date,pr_grade.gr_name, pr_emp_com_info.gross_sal,pr_emp_add.*,pr_emp_edu.*');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_grade');
		$this->db->from('pr_emp_add');
	...
	function grid_daily_allowance_bills($att_date, $grid_emp_id)
	{
		...
		$data = array();
		$this->db->distinct();
		$this->db->select('pr_emp_com_info.emp_id,pr_emp_per_info.emp_full_name,pr_designation.desig_name, pr_emp_com_info.emp_join_date, pr_dept.dept_name, pr_section.sec_name,pr_section.sec_id, pr_line_num.line_name, pr_id_proxi.proxi_id, pr_emp_shift.shift_name,pr_emp_com_info.emp_cat_id, pr_emp_com_info.gross_sal, pr_emp_com_info.ot_entitle, pr_emp_add.emp_pre_add, pr_emp_shift_log.in_time, pr_emp_shift_log.out_time, pr_emp_shift_log.ot_hour, pr_emp_shift_log.extra_ot_hour');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_designation');
		$this->db->from('pr_dept');
		...
		foreach($query->result() as $rows)
		{
			...
			$out_time = date('h:i:s A', strtotime($rows->out_time));
			
			//==========================Night Allowance=================================================
			//==========================================================================================
			$night_allowance_rules 			= $this->get_night_allowance_rules($rows->sec_id);
			if($night_allowance_rules['msg'] == "OK" )
			{
				$night_allowance_rules_data 	= $this->night_allowance_rules_data($night_allowance_rules['rules_id'] );
				$night_allowance_time 			= $night_allowance_rules_data ['night_allowance_time'];
				
				$night_out_time_median 	= date('A', strtotime($night_allowance_time));
				$out_time_median 		= date('A', strtotime($rows->out_time));
				if($night_out_time_median != $out_time_median)//==
				{
					$night_allowance_amount =	$night_allowance_rules_data ['night_allowance_amount'] ;
					$tiffin_amount 			= 0;
				}
				else
				{
					//$night_allowance_amount 	= 0;
					if($night_allowance_time <= $rows->out_time)
					{
						$night_allowance_amount 			= $night_allowance_rules_data ['night_allowance_amount'];
					}
					else
					...
					$tiffin_allowance_rules 			= $this->get_tiffin_allowance_rules($rows->sec_id);
					if($tiffin_allowance_rules['msg'] == "OK" )
					{
						
						$tiffin_allowance_rules_data 	= $this->tiffin_allowance_rules_data($tiffin_allowance_rules['rules_id'] );
						$tiffin_time 					= $tiffin_allowance_rules_data ['tiffin_allowance_time'];
						
					}
					if($tiffin_time <= $rows->out_time)
					{
						$tiffin_amount 			= $tiffin_allowance_rules_data ['tiffin_allowance_amount'];
					}
					else
					{
						$tiffin_amount 			= 0;
			...
			if ($rows->out_time == "00:00:00")
			{
				//echo $rows->out_time;
				$tiffin_amount 			= 0;
				$night_allowance_amount 	= 0;
			}
			if($night_allowance_amount 	!= 0)
			{
				$tiffin_amount 			= 0;
			}
			
			
			if($tiffin_amount == 0 &&   $night_allowance_amount ==0)
			{
				 continue;
			}
			...
			$data["emp_shift"][] 				= $rows->shift_name;
			$data["out_time"][] 				= $rows->out_time; 
			$data["night_allowance_amount"][] 	= $night_allowance_amount ;
			$data["tiffin_amount"][]		 	= $tiffin_amount ;
	...
	function get_tiffin_allowance_rules($sec_id)
	{
		$this->db->select('rules_id');
		$this->db->from('pr_tiffin_allowance_level');
	...
	function tiffin_allowance_rules_data($tiffin_allowance_rules)
	{
		...
		$this->db->where("rules_id", $tiffin_allowance_rules);
		$query = $this->db->get();
		if($query->num_rows()>0)
		{
			$row = $query->row();
			$data['tiffin_allowance_amount'] = $row->allowance_amount;
			$data['tiffin_allowance_time'] = $row->allowance_time;
			$data['msg'] = "OK";
		}
		else
		{
	...
	function night_allowance_rules_data($night_allowance_rules)
	{
		// $night_allowance_rules_amount = $this->db->where("designation_id",$emp_desi_id)->get('pr_night_allowance_level')->row()->rules_id;
		//return $night_allowance_rules_amount;
		$this->db->select('*');
		...
		{
			$row = $query->row();
			$data['night_allowance_amount'] = $row->allowance_amount;
			$data['night_allowance_time'] = $row->allowance_time;
			$data['msg'] = "OK";
		}
		else
		{
	...
	function get_night_allowance_rules($sec_id)
	{
		$this->db->select('rules_id');
	...
	function grid_monthly_ot_register($grid_firstdate, $grid_emp_id)
	{
		...
		$this->db->select('pr_emp_com_info.emp_id,pr_emp_per_info.emp_full_name,  pr_designation.desig_name, pr_emp_com_info.emp_join_date, pr_dept.dept_name, pr_section.sec_name, pr_line_num.line_name, pr_id_proxi.proxi_id, pr_emp_shift.shift_name,pr_emp_com_info.emp_cat_id, pr_emp_com_info.gross_sal,pr_emp_com_info.emp_shift');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_designation');
		$this->db->from('pr_dept');
   ...
   function get_allowance_data($emp_id,$ot_entitle,$att_date)
	...
	function grid_continious_absent_list($year, $month, $date, $status, $grid_emp_id,$grid_second_data)
	{
		
		$sSecondDate = date("Y-m-d", strtotime($grid_second_data)); 
		$data = array();
		$day = $year."-".$month."-".$date;
		$date_range = $this->get_no_of_days($day,$sSecondDate);
		$this->db->select('emp_id');
		$this->db->where('shift_log_date', $day);
		$this->db->where_in('emp_id', $grid_emp_id);
		$query = $this->db->get('pr_emp_shift_log');
		...
		foreach($query->result() as $rows)
		{
			...
			foreach($query->result() as $rows)
			{
				...
				$limit_days = $this->common_model->get_setup_attributes(10);
				$emp_num_rows = $this->attendance_check_for_absent($emp_id,$status,$limit_days,$day,$sSecondDate);
				//echo "$day===$sSecondDate===$emp_id===$emp_num_rows<br></br>";
				if($emp_num_rows != $date_range )
				{
					continue;
				}
	...
	function attendance_check_for_absent($emp_id,$present_status,$limit_days,$start_date,$sSecondDate)
	{
		//echo "$present_status=> $num_of_days, $start_date###";
		$num_of_days 	= $this->get_no_of_days($start_date,$sSecondDate);
