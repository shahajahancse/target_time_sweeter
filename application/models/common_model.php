...
class Common_model extends CI_Model{
	...
	function salary_structure($gross_salary)
	{
		...
		$data['gross_salary']  = $gross_salary;
		$data['basic_sal'] 	   = round(($gross_salary - 1850) / 1.5);
		$data['house_rent']    = round($gross_salary - 1850 - $data['basic_sal']);
		$data['ot_rate']       = round(($data['basic_sal'] * 2  / 208),2);
	...
	{ 
		...
		for ($counter = 1; $counter <= $page; $counter ++)
		{
	 	...
	 	$result_return_arr = array(
	 		'Gtotal_gross_salary' => $Gtotal_gross_salary,
	 		'total_mday'		  => $Gtotal_mday,
	 		'total_wday'		  => $Gtotal_wday,
	 		'total_offday'		  => $Gtotal_offday,
	 		'total_absday'		  => $Gtotal_absday,
	 		'total_baabsday'	  => $Gtotal_baabsday,
	 		'total_clday'		  => $Gtotal_clday,
	 		'total_slday'		  => $Gtotal_slday,
	 		'total_basic_salary'  => $Gtotal_basic_salary,
	 		'total_transpot_allowance_all' => $Gtotal_transpot_allowance_all,
	 		'total_food_allowance_all' => $Gtotal_food_allowance_all,
	 		'total_net_wages_after_deduction' => $grand_total_net_wages_after_deduction,
	 		'total_payday'		  => $Gtotal_payday,
	 		'total_subsidiary'	  => $Gtotal_subsidiary,
	 		'total_att_bonus'	  => $grand_total_att_bonus,
	 		'total_fhday'	      => $Gtotal_fhday,
	 		'total_friday_amount' => $grand_total_friday_amount,
	 		'total_nday'		  => $Gtotal_nday,
	 		'total_fhday'		  => $Gtotal_fhday,
	 		'total_nowday'		  => $Gtotal_nowday,
	 		'total_nw_amount'	  => $grand_total_nw_amount,
	 		'Tbaftera_deduct'	  => $GTbaftera_deduct,
	 		'total_pd_amount'	  => $grand_total_pd_amount,
	 		'total_pb_amount'	  => $grand_total_pb_amount,
	 		'total_pay_amount'	  => $gtotal_pay_amount,
	 		'total_stamp'		  => $grand_total_stamp,
	 		'grand_total_net_wages_with_ot' => $grand_total_net_wages_with_ot
	 	);
	 	return $result_return_arr;
	}
	//end get_ultimate_salary function


	function get_ultimate_salary_new($value, $salary_month, $grid_status)
	{
			$date = $salary_month;
			$sal_year=trim(substr($date,0,4));
			$sal_month=trim(substr($date,5,2));
	...
	$date_format = date("F-Y", mktime(0, 0, 0, $sal_month, $day, $sal_year));
		...
		for($p=0; $p<=$per_page_row;$p++)
		{
			...
			$total_pay = ($subsidary + $pd_amount  + $att_bonus + $value[$k]->pd_bonus_amount+$value[$k]->night_allowance+$value[$k]->holiday_allowance+$value[$k]->none_work_allowance) - $stam_value - $adv_deduct;
	  ...
	  $result_return_arr = array(
	 		...
	 		'total_pay_amount'	  => $gtotal_pay_amount,
	 		'total_stamp'		  => $grand_total_stamp,
			'grand_total_advance'=> $grand_total_advance_salary,
	 		'grand_total_net_wages_with_ot' => $grand_total_net_wages_with_ot
	 	);
	 	return $result_return_arr;
