		...
		foreach($all_emp_id->result() as $rows)
		{
			...
			{ 
				...
				if($resign_check != false && $salary_month == $join_month)
				{
					...
					$n_doj = $rows->emp_join_date;
					$join_month = trim(substr($n_doj,0,7));
					$doj_data = $this->get_join_month_dates($n_doj);
					...
					$data["before_after_absent_days"] = $before_after_absent;
					$data["absent_days"] = $data["absent_days"] - $before_after_absent;
					//$doj_before_after_absent_amount = $gross_sal / ($num_of_days-($weeked+$holiday)) * $before_after_absent;
					$doj_before_after_absent_amount = $gross_sal / $num_of_days * $before_after_absent;
					$data["before_after_absent_amount"] = $doj_before_after_absent_amount;
					
					$abs_deduction = $basic_sal / ($cmonth_days-($weeked+$holiday)) * $data["absent_days"];
				
				    $abs_deduction_com = $basic_sal / ($cmonth_days-($weeked+$holiday)) * $data["absent_days"];
