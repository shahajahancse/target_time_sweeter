...
class Production_salary_process_model extends CI_Model{
	...
	function production_pay_sheet($year, $month,$process_check)
	{
		...
		foreach($all_emp_id->result() as $rows)
		{
			...
			{ 
				...
				$ot_rate_com			= round($gross_sal/208,2);
				...
				if($num_rows != 0)
				{
					foreach ($query->result() as $row_man_entry)
					{
					  ...
					  $baa_day 			    = $row_man_entry->baa_day;
					  $leave_cl 			= $row_man_entry->leave_cl;
					  $leave_sl 			= $row_man_entry->leave_sl;
					  $leave_ml				= $row_man_entry->leave_ml;
					  $leave_el 			= $row_man_entry->leave_el;
				...
				$data_com["before_after_absent_days"] = $baa_day;
				$data_com["c_l"] 		= $leave_cl;
				$data_com["s_l"] 		= $leave_sl;
				$data_com["e_l"] 			= $leave_el;
				$data_com["m_l"] 			= $leave_ml;
				...
				//****New Deduction Developed By Tarek on 6-2-17
				/*echo $num_of_days;
				echo $weeked;
				echo $holiday;exit(); */
				$cmonth_days = 30;
				$resign_check = $this->resign_check_2($emp_id, $salary_month);
				$left_check = $this->left_check_2($emp_id, $salary_month);

				if($resign_check != false && $salary_month == $join_month)
				{
					
					$data["before_after_absent_days"] = $baa_day;
					$doj_before_after_absent_amount = $gross_sal / $num_of_days * $baa_day;
					$data["before_after_absent_amount"] = $doj_before_after_absent_amount;
					$abs_deduction = $basic_sal / $cmonth_days * $data["absent_days"];
				    $abs_deduction_com = $basic_sal / $cmonth_days * $data["absent_days"];
					
				}
				elseif($left_check != false && $salary_month == $join_month)
				{
				...
				elseif($salary_month == $join_month)
				{
					...
					$data["before_after_absent_days"] = 0;
					$data["before_after_absent_amount"] = 0;
				  }
				  
				  //$abs_deduction = $basic_sal / ($num_of_days-($weeked+$holiday)) * $data["absent_days"];
				  //$abs_deduction_com = $basic_sal / ($num_of_days-($weeked+$holiday)) * $data["absent_days"];
                ...
                $baabs = $this->db->select('before_after_absent_days')->where('emp_id',$emp_id)->where('salary_month',$start_date)->get('pd_pay_scale_sheet_com')->row_array();
				 
				if($absent == 0 and $total_leave == 0 and $baabs['before_after_absent_days'] == 0)
				{
					if($attend != 0){
	...
	function get_join_month_dates($n_doj)
	{
		//echo $n_doj;
		$data_n = array();
		$year 		= date('Y', strtotime($n_doj));
		$month 		= date('m', strtotime($n_doj));
		$day 		= date('d', strtotime($n_doj));
		$last_day 	= date('t', strtotime($n_doj));
		
		$data_n['doj_1st_date'] 	= date("Y-m-d", mktime(0, 0, 0, $month, 1, $year));
		$data_n['doj_2nd_date'] 	= date("Y-m-d", strtotime("-1 day",strtotime($n_doj)));
		$data_n['doj_1st_count'] 	= date("d", strtotime($data_n['doj_2nd_date']));
		$data_n['doj_3rd_date'] 	= $n_doj;
		$data_n['doj_2nd_count'] 	= $last_day;
		$data_n['doj_4th_date'] 	= date("Y-m-d", mktime(0, 0, 0, $month, $last_day, $year));
	...
	function resign_check_2($emp_id, $salary_month)
	{
	...
	function left_check_2($emp_id, $salary_month)
	{
