...
class manual_attendance_model extends CI_Model{
	...
	function get_info($emp_id,$month,$year)
	{
		...
		if($num_rows != 0)
		{
		  foreach ($query->result() as $row)
		  {
			...
			$data['baa_day'] 			= $row->baa_day;
			$data['leave_cl'] 			= $row->leave_cl;
			$data['leave_sl'] 			= $row->leave_sl;
			$data['leave_ml'] 			= $row->leave_ml;
			$data['leave_el'] 			= $row->leave_el;
			$data['no_work'] 			= $row->no_work;
			$data['night'] 				= $row->night;//11
			$data['late'] 				= $row->late;
			$data['h_day'] 				= $row->h_day;//13
			$data['rows'] 				= $num_rows;//14
	...
	function manual_attendance_db()
	{
		...
		$baa_day 			= $this->input->post('baa_day');
		$leave_cl 			= $this->input->post('leave_cl');
		$leave_sl 			= $this->input->post('leave_sl');
		$leave_ml			= $this->input->post('leave_ml');
		$leave_el			= $this->input->post('leave_el');
		...
		$data['p_day'] 				= $p_day;
		$data['a_day'] 				= $a_day;
		$data['baa_day'] 			= $baa_day;
		$data['leave_cl'] 			= $leave_cl;
		$data['leave_sl'] 			= $leave_sl;
		$data['leave_ml'] 			= $leave_ml;
		$data['leave_el'] 			= $leave_el;
