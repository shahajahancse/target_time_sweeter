	...
	{		
		...
		for($i = 0 ; $i < $count - 1; $i++) {
			...
			***********************************/
			...
			$line_name	= trim($linearray[6]);
			$pos_name	= trim($linearray[7]);
			...
			$salary_type= trim($linearray[11]);
			$sal_gross	= trim($linearray[12]);
			$bonus_name	= trim($linearray[13]);
			
			$gender		= trim($linearray[14]);
			$marital	= trim($linearray[15]);
			$ot			= trim($linearray[16]);
			
			if($salary_type == "Production")
			{
				$salary_type = 2;
			}
			else
			...
			$pre_add	= '';//trim($linearray[17]);
			$par_add	= '';//trim($linearray[18]);
			
			if($gender 	=='F'  ){ $gender 	= 2; }else{ $gender = 1; }
			if($marital =='MARRIED'){ $marital 	= 2; }else{ $marital= 1; }
			if($ot 		=='Yes'){ $ot 		= 0; }else{ $ot 	= 1; }
			
			$this->check_dept($dept_name);
			$this->check_section($sec_name);
			$this->check_line($line_name);
			$this->check_designation($desig_name);
			$this->check_position($pos_name);
			//$this->check_floor($floor_name);
			//$this->check_block($block_name);
			...
			$posi_id 		= $this->get_position_id_by_name($pos_name);
			$sal_grade_id 	= $this->get_salary_grade_id_by_name($sal_grade);
			$bonus_id 		= $this->get_bonus_id_by_name($bonus_name);
			//$floor_id		= $this->get_floor_id_by_name($floor_name);
			//$block_id		= $this->get_block_id_by_name($block_name);
			...
			if($addauto){
				...
				echo $query2 = "INSERT INTO pr_emp_com_info (`emp_id`, `emp_dept_id`, `emp_sec_id`, `emp_line_id`, `emp_desi_id`,`emp_operation_id`,`emp_position_id`, `emp_sal_gra_id`, `emp_cat_id`, `emp_shift`, `gross_sal`, `ot_entitle`, `transport`, `lunch`, `att_bonus`, `salary_draw`,`salary_type`,`emp_join_date`) 
								VALUES ('$emp_id',$dept_id,$sec_id,$line_id,$desig_id,0,$posi_id, '$sal_grade_id',1,1,'$sal_gross',$ot,1,1,'$bonus_id',1,'$salary_type','$doj1' );";
				echo "<br>";
...
<?php
	...
	function check_dept($dept_name)
	{
		$num_row = $this->db->where('dept_name',trim($dept_name))->get('pr_dept')->num_rows();
		if($num_row < 1)
		{
	...
	function check_line($line_name)
	{
		$num_row = $this->db->where('line_name',trim($line_name))->get('pr_line_num')->num_rows();
		if($num_row < 1)
		{
	...
	function check_position($pos_name)
	{
		$num_row = $this->db->where('posi_name',trim($pos_name))->get('pr_emp_position')->num_rows();
		if($num_row < 1)
		{
			...
			$this->db->insert('pr_emp_position', $data); 
		}
	}
	...
	{
		$num_row = $this->db->where('floor_name',trim($floor_name))->get('pr_emp_floor')->num_rows();
		if($num_row < 1)
		{
			...
			$this->db->insert('pr_emp_floor', $data); 
		}
	}
	function check_block($block_name)
	{
		$num_row = $this->db->where('block_name',trim($block_name))->get('pr_emp_block')->num_rows();
		if($num_row < 1)
		{
			...
			$this->db->insert('pr_emp_block', $data); 
		}
	}
	*/
	...
	function get_position_id_by_name($posi_name)
	{
		$this->db->select('posi_id');
		$this->db->where('posi_name',trim($posi_name));
		$query = $this->db->get('pr_emp_position');
		$row = $query->row();
		return $posi_id = $row->posi_id;
	}
	...
	/*
	function get_floor_id_by_name($floor_name)
	{
		$this->db->select('floor_id');
		$this->db->where('floor_name',trim($floor_name));
		$query = $this->db->get('pr_emp_floor');
		$row = $query->row();
		return $floor_id = $row->floor_id;
	}
	
	function get_block_id_by_name($block_name)
	{
		$this->db->select('block_id');
		$this->db->where('block_name',trim($block_name));
		$query = $this->db->get('pr_emp_block');
		$row = $query->row();
