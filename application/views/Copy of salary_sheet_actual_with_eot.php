...
?>
  ...
  <tr height="20px">
    ...
    <td width="32" height="20px"><div align="center"><strong>OT + EOT</strong></div></td>
...
<?php
	...
	for($p=0; $p<=$per_page_row;$p++)
	{
		...
		echo "<td>";
		$gross = $value[$k]->gross_sal;
		$absent_days = $value[$k]->absent_days;
		$num_of_days = $value[$k]->total_days;
		echo $absent_deduction = round($gross / $num_of_days * $absent_days);
		...
		$pay_days = $value[$k]->pay_days;
		echo $salary_monthly = round($gross / $num_of_days * $pay_days);
		$total_pay_wages  = $total_pay_wages  + $salary_monthly;
		echo "</td>";
				
		echo "<td>";
		$total_ot_eot = $value[$k]->ot_hour + $value[$k]->eot_hour;
		print_r ($value[$k]->ot_hour."+".$value[$k]->eot_hour."=".$total_ot_eot);
		//echo "ot".$row->ot_hour;
		$ot_hour = $ot_hour + $value[$k]->ot_hour; 
		$eot_hour = $eot_hour + $value[$k]->eot_hour; 
		
		$total_ot_hours   = $total_ot_hours + $value[$k]->ot_hour+ $value[$k]->eot_hour; 
		echo "</td>";
		...
		echo "<td>";
		$total_ot_eot_amount = round($total_ot_eot * $value[$k]->ot_rate);
		//print_r ($value[$k]->ot_amount);
		echo $total_ot_eot_amount;
		//$ot_charge = $value[$k]->ot_amount;
		//$ot_amount = $ot_amount + $ot_charge; 
		$total_ot_amount   = $total_ot_eot_amount + $total_ot_amount; 
		echo "</td>";
				
		echo "<td>";
		print_r ($value[$k]->att_bonus);
		$att_bonus_charge = $value[$k]->att_bonus;
		//echo "a_b".$row->att_bonus;
		$att_bonus = $att_bonus + $att_bonus_charge;
		$total_att_bonus   = $total_att_bonus + $att_bonus_charge; 
		echo "</td>";
		
		echo "<td>";
		//print_r ($value[$k]->gross_pay);
		echo $gross_payable = $salary_monthly + $total_ot_eot_amount + $att_bonus_charge;
		//echo "g_p".$row->gross_pay;
		$gross_pay = $gross_pay + $gross_payable; 
		$total_gross_pays   = $total_gross_pays + $gross_payable; 
		echo "</td>";
		...
		{
			echo $stam_value;
			$gross_salary = $gross_payable - $stam_value; 
		...
		$adv_deduct_charge = $value[$k]->adv_deduct;
		//echo "ad".$row->adv_deduct;
		$adv_deduct = $adv_deduct + $adv_deduct_charge; 
		echo "</td>";
		...
		echo "<td>";
		echo $gross_salary = $gross_salary - $adv_deduct_charge;
