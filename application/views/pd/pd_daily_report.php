...
<table border="1" cellpadding="0" cellspacing="0" style="font-size:13px; border-collapse:collapse">
  ...
  <?php
  	$query = $this->db->select('article_id_pk as article_id,article_id as article,buyer_id')->where('status','Enable')->get('pd_style_infos');
	...
	foreach($query->result() as $rows){ ?>
		<?php
            ...
            <tr>
                <td> <?php echo $i;?> </td>
                <td> <?php echo $buyer_name = $this->db->select('buyer_name')->where('buyer_id',$rows->buyer_id)->get('pd_buyer_infos')->row()->buyer_name;?> </td>
                <td> <?php echo $rows->article; $article_id = $rows->article_id;?> </td>
                <td> 
					<?php echo $color_name = $this->db->select('color_name')->where('color_id',$color_rows->color_id)->get('pd_color_infos')->row()->color_name; ?> 
                
                </td>
                <td align="center"> 
					<?php echo $order_number = $this->db->select('order_number')->where('article_id_pk',$rows->article_id)->get('pd_style_infos')->row()->order_number; ?> 
                ...
                <td> 
                 ...
                 <td> 
					<?php $gauge_id = $this->db->select('gauge_id')->where('article_id_pk',$rows->article_id)->get('pd_style_infos')->row()->gauge_id; echo $gauge_name = $this->db->select('gauge_name')->where('gauge_id',$gauge_id)->get('pd_gauge_infos')->row()->gauge_name; ?> 
