...
<?php
	...
	for($p=0; $p<=$per_page_row;$p++)
	{
		...
		$total_baabsday = $total_baabsday + $value[$k]->before_after_absent;
		//echo "abs".$row->absent_days;
		echo "</td>";
		...
		$Gtotal_offday = $Gtotal_offday + $value[$k]->weeked;
		$Gtotal_absday  = $Gtotal_absday + $value[$k]->absent_days;
		$Gtotal_baabsday = $Gtotal_baabsday + $value[$k]->before_after_absent;
