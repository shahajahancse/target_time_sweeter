...
<?php
	...
	for($p=0; $p<=$per_page_row;$p++)
	{
		...
		$net_wages_after_deduction = $gross_sal - $abs_deduct - $adv_deduct - $deduct_amount + $att_bonus;
		...
		echo "<td>";
		echo $subsidary = $gross_sal -  ( $pd_amount +  $value[$k]->none_work_allowance );
		echo "</td>";
		...
		echo $total_pay = $pd_amount  + $att_bonus + $value[$k]->pd_bonus_amount+$value[$k]->night_allowance+$value[$k]->holiday_allowance+$value[$k]->none_work_allowance - $stam_value;
		echo "</td>";
